# Open Hardware Control by Frelidon 3.4.29.3 INTERN
...
Lade `open-hardware-control-3.4.29.3-0.intern2.noarch.rpm` in deinen Downloads-Ordner und führe aus:

```bash
cd ~/Downloads
sudo dnf install ./open-hardware-control-3.4.29.3-0.intern2.noarch.rpm
```

### Debian, Ubuntu und Linux Mint – DEB

Lade `open-hardware-control_3.4.29.3~intern2_all.deb` in deinen Downloads-Ordner und führe aus:

```bash
cd ~/Downloads
sudo apt install './open-hardware-control_3.4.29.3~intern2_all.deb'
```

### Universelles Installationspaket

Das ZIP funktioniert auf Fedora/Nobara, Debian/Ubuntu/Mint, Arch/Manjaro/EndeavourOS und openSUSE. Lade `open_hardware_control_v3_4_29_3_INTERN.zip` herunter und führe aus:

```bash
cd ~/Downloads
unzip open_hardware_control_v3_4_29_3_INTERN.zip
cd open-hardware-control-3.4.29.3-INTERN
chmod +x install.sh
./install.sh
```
