...
7. Read the relevant latest release notes and `docs/CHANGELOG.md`.
...
- Main application file: `src/kraken_control.py` (historical filename retained intentionally).
...
- Add a `docs/CHANGELOG.md` entry for user-visible changes and release notes for a versioned release.
...
- New or migrated feature modules live only in their current `src/modules/<name>/v<major>_<minor>/` folder and follow `docs/project/MODULE_REGISTRY.md`.
