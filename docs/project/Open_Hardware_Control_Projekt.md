...
Version 3.4.29.46 beendet den minütlichen Start von `openrgb --client --list-devices`, solange das Hauptfenster im Infobereich verborgen ist und weder ein RGB-Startprofil noch eine Schreibfreigabe oder eine geplante Wiederholung ausstehen. Das Repository ist in `docs/project`, `docs/ai`, `docs/hardware`, `docs/security`, `docs/releases`, `packaging/` und `.github/` sortiert; die installierte Programmstruktur bleibt flach und unverändert. Die README zeigt eine anklickbare Galerie mit sieben aktuellen Screenshots, die Installation direkt nach dem Levita-Highlight und nur noch die letzten vier Versionen.

Version 3.4.29.45 hält ein gespeichertes RGB-Startprofil während einer vorläufigen OpenRGB-Teilerkennung vorgemerkt, wartet begrenzt auf den vollständigen Gerätebestand und startet danach das Design. Die Modulregisterprüfung folgt nun dem tatsächlichen Build-Kanal, sodass der zuvor intern geprüfte 3.4.29-Stand stabil veröffentlicht werden kann. Der plattformunabhängige Vorschau-Warteschlangentest isoliert seine synthetische Last vollständig von bereits gestarteten Aufgaben für das mitgelieferte OHC-Video.
