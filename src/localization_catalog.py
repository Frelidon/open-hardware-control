...
# 3.4.29.42 monitor placement and Wallpaper Engine controls.
UI_TRANSLATIONS["en"].update({
    "Startbildschirm des Programmfensters": "Application window startup screen",
    "Programmfenster starten auf": "Start application window on",
    "Hauptbildschirm (Standard)": "Primary screen (default)",
    "Skalierung:": "Scaling:", "Wallpaper-Skalierungsmodus": "Wallpaper scaling mode",
    "Seitenverhältnis beibehalten": "Keep aspect ratio",
    "Skalieren und zuschneiden": "Scale and crop",
    "Auf Vollbild strecken": "Stretch to full screen",
    "Skalierung anwenden": "Apply scaling",
})
UI_TRANSLATIONS["es"].update({
    "Startbildschirm des Programmfensters": "Pantalla de inicio de la ventana",
    "Programmfenster starten auf": "Iniciar la ventana en",
    "Hauptbildschirm (Standard)": "Pantalla principal (predeterminada)",
    "Skalierung:": "Escala:", "Wallpaper-Skalierungsmodus": "Modo de escala del fondo",
    "Seitenverhältnis beibehalten": "Mantener relación de aspecto",
    "Skalieren und zuschneiden": "Escalar y recortar",
    "Auf Vollbild strecken": "Estirar a pantalla completa",
    "Skalierung anwenden": "Aplicar escala",
})
UI_TRANSLATIONS["fr"].update({
    "Startbildschirm des Programmfensters": "Écran de démarrage de la fenêtre",
    "Programmfenster starten auf": "Démarrer la fenêtre sur",
    "Hauptbildschirm (Standard)": "Écran principal (par défaut)",
    "Skalierung:": "Mise à l’échelle :", "Wallpaper-Skalierungsmodus": "Mode de mise à l’échelle du fond",
    "Seitenverhältnis beibehalten": "Conserver les proportions",
    "Skalieren und zuschneiden": "Mettre à l’échelle et recadrer",
    "Auf Vollbild strecken": "Étirer en plein écran",
    "Skalierung anwenden": "Appliquer la mise à l’échelle",
})
...
HELP_TOPICS: dict[str, dict[str, dict[str, object]]] = {
    "de": {
        ...
        "supported_devices": _help_topic("Unterstützte Geräte", "OHC unterscheidet feste Geräteprofile und dynamisch von lokalen Backends gemeldete Hardware.", ["NZXT Kraken: Kraken 2023 (1e71:300e), Kraken Elite 2024 RGB (3012) und Kraken Plus (3014) sind im Geräteverzeichnis unterstützt.", "Kraken 2023 Elite (300c) wird wegen des bekannten Treiberstatus nur sicher erkannt; Schreibzugriffe bleiben gesperrt.", "NZXT 2023 RGB Controller (2012) wird über liquidctl gesteuert.", "Thermalright Levita Vision 360 ARGB Black: 1600×720-Display über TRCC Linux; Pumpe/Radiatorlüfter über bestätigte hwmon-Kanäle.", "Weitere RGB- und Corsair-Geräte folgen den dynamischen Listen der lokal installierten OpenRGB- bzw. OpenLinkHub-Version.", "Mainboard-/Gehäuselüfter benötigen kompatibles Linux-hwmon-PWM und eine einzelne physische Bestätigung; GPU-Lüfter bleiben unangetastet."], 6),
        "wallpaper_engine": _help_topic("Wallpaper Engine", "Ein wiederaufrufbarer Assistent führt durch Steam, Workshop und das lokal installierte CaptSilver-Plasma-Plugin.", ["Installiere Wallpaper Engine über Steam und abonniere zunächst mindestens fünf Workshop-Wallpaper.", "Auf Fedora kann OHC nach zwei Bestätigungen das passende offizielle RPM herunterladen, per GitHub-SHA256 prüfen und über Polkit/DNF installieren; OHC erhält kein Passwort.", "Setze die Steam-Bibliothek auf den Ordner, der steamapps enthält; normalerweise ~/.local/share/Steam. Eigene Videos gehören nur in einen kleinen separaten Ordner.", "Wähle Wallpapers oder Videos direkt in OHC; Spezialoptionen bleiben in der originalen Plasma-Oberfläche erreichbar.", "Der Originalzustand ist Standard. Die optionale Optimierung kann vollständig zurückgesetzt werden."], 9),
        "desktop": _help_topic("Desktop-Designs", "Die experimentellen Desktop-Designs ändern ausgewählte KDE-Oberflächenbausteine mit Sicherungs- und Rückweg.", ["Aktiviere den experimentellen Bereich in den Einstellungen.", "Prüfe vor dem Anwenden die geplanten Änderungen.", "OHC legt vor Änderungen eine Sicherung an und bietet eine Wiederherstellung an."], 10),
    },
    "en": {
        ...
        "supported_devices": _help_topic("Supported devices", "OHC distinguishes fixed device profiles from hardware reported dynamically by local backends.", ["NZXT Kraken: Kraken 2023 (1e71:300e), Kraken Elite 2024 RGB (3012) and Kraken Plus (3014) are supported registry entries.", "Kraken 2023 Elite (300c) is detected safely only; writes remain disabled because of its known driver status.", "The NZXT 2023 RGB Controller (2012) is controlled through liquidctl.", "Thermalright Levita Vision 360 ARGB Black: 1600×720 display through TRCC Linux; pump/radiator fans through confirmed hwmon channels.", "Additional RGB and Corsair devices follow the dynamic lists of the locally installed OpenRGB and OpenLinkHub versions.", "Mainboard/case fans require compatible Linux hwmon PWM and individual physical confirmation; GPU fans are never changed."], 6),
        "wallpaper_engine": _help_topic("Wallpaper Engine", "A reopenable assistant covers Steam, Workshop and the locally installed CaptSilver Plasma plugin.", ["Install Wallpaper Engine through Steam and initially subscribe to at least five Workshop wallpapers.", "On Fedora, OHC can download the matching official RPM, verify GitHub's SHA256 digest and install through Polkit/DNF after two confirmations; OHC never receives the password.", "Point Steam Library at the folder containing steamapps; usually ~/.local/share/Steam. Personal videos belong only in a small separate folder.", "Select wallpapers or videos in OHC; advanced options remain available in Plasma's original interface.", "The original state is the default. The optional optimisation can be reverted completely."], 9),
        "desktop": _help_topic("Desktop designs", "Experimental desktop designs modify selected KDE appearance parts with backup and rollback.", ["Enable the experimental area in Settings.", "Review the planned changes before applying them.", "OHC creates a backup before modifications and offers restoration."], 10),
    },
    "es": {
        ...
        "supported_devices": _help_topic("Dispositivos compatibles", "OHC distingue perfiles fijos de los dispositivos que comunican dinámicamente los servicios locales.", ["NZXT Kraken: Kraken 2023 (1e71:300e), Kraken Elite 2024 RGB (3012) y Kraken Plus (3014) figuran como compatibles.", "Kraken 2023 Elite (300c) solo se detecta de forma segura; la escritura permanece bloqueada por el estado conocido del controlador.", "NZXT 2023 RGB Controller (2012) se controla mediante liquidctl.", "Thermalright Levita Vision 360 ARGB Black: pantalla 1600×720 mediante TRCC Linux; bomba/ventiladores mediante canales hwmon confirmados.", "Otros dispositivos RGB y Corsair siguen las listas dinámicas de las versiones locales de OpenRGB y OpenLinkHub.", "Los ventiladores de placa/caja requieren PWM hwmon compatible y confirmación física; OHC no modifica los ventiladores GPU."], 6),
        "wallpaper_engine": _help_topic("Wallpaper Engine", "Un asistente reutilizable explica Steam, Workshop y el complemento local CaptSilver para Plasma.", ["Instala Wallpaper Engine con Steam y suscríbete inicialmente a cinco fondos del Workshop.", "En Fedora, OHC puede verificar el SHA256 publicado e instalar el RPM oficial mediante Polkit/DNF tras dos confirmaciones; OHC no recibe la contraseña.", "Selecciona como biblioteca de Steam la carpeta que contiene steamapps y usa una carpeta pequeña separada para vídeos propios.", "Selecciona fondos o vídeos en OHC; las opciones avanzadas siguen en la interfaz original de Plasma.", "El estado original es el predeterminado y la optimización opcional se puede revertir."], 9),
        "desktop": _help_topic("Diseños de escritorio", "Los diseños experimentales modifican partes seleccionadas de KDE con copia y restauración.", ["Activa el área experimental en Ajustes.", "Revisa los cambios antes de aplicarlos.", "OHC crea una copia antes de modificar y ofrece restauración."], 10),
    },
    "fr": {
        ...
        "supported_devices": _help_topic("Appareils pris en charge", "OHC distingue les profils fixes du matériel signalé dynamiquement par les services locaux.", ["NZXT Kraken : Kraken 2023 (1e71:300e), Kraken Elite 2024 RGB (3012) et Kraken Plus (3014) sont pris en charge dans le registre.", "Kraken 2023 Elite (300c) est uniquement détecté en sécurité ; les écritures restent bloquées à cause de l’état connu du pilote.", "Le NZXT 2023 RGB Controller (2012) est piloté via liquidctl.", "Thermalright Levita Vision 360 ARGB Black : écran 1600×720 via TRCC Linux ; pompe/ventilateurs via des canaux hwmon confirmés.", "Les autres appareils RGB et Corsair suivent les listes dynamiques des versions OpenRGB et OpenLinkHub installées.", "Les ventilateurs carte mère/boîtier exigent un PWM hwmon compatible et une confirmation physique ; les ventilateurs GPU ne sont jamais modifiés."], 6),
        "wallpaper_engine": _help_topic("Wallpaper Engine", "Un assistant réutilisable explique Steam, Workshop et le module Plasma CaptSilver installé localement.", ["Installez Wallpaper Engine via Steam et abonnez-vous d’abord à cinq fonds Workshop.", "Sous Fedora, OHC peut vérifier le SHA256 publié puis installer le RPM officiel via Polkit/DNF après deux confirmations ; OHC ne reçoit pas le mot de passe.", "Choisissez comme bibliothèque Steam le dossier contenant steamapps et utilisez un petit dossier séparé pour vos vidéos.", "Sélectionnez les fonds ou vidéos dans OHC ; les options avancées restent dans l’interface Plasma originale.", "L’état original est la valeur par défaut et l’optimisation facultative est réversible."], 9),
        "desktop": _help_topic("Designs du bureau", "Les designs expérimentaux modifient certains éléments KDE avec sauvegarde et retour arrière.", ["Activez l’espace expérimental dans Paramètres.", "Vérifiez les changements prévus avant application.", "OHC crée une sauvegarde avant modification et propose une restauration."], 10),
    },
}
